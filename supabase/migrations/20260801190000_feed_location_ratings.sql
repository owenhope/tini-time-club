-- Include each venue's current TTC aggregate on every review row. The
-- top-level RPC return type stays the same; only the embedded location JSON is
-- enriched, so existing clients remain compatible.
...
GRANT ALL ON FUNCTION public.feed_reviews(uuid, integer, integer, uuid, bigint, boolean)
  TO authenticated;
GRANT ALL ON FUNCTION public.feed_reviews(uuid, integer, integer, uuid, bigint, boolean)
  TO service_role;
