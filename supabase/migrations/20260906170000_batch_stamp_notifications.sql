...
-- A reconcile that lands many stamps at once (an existing member's first
-- 4.2.0 open, or a catalog expansion) collapses into ONE summary
-- notification instead of one per stamp — the notifications webhook sends
-- a push per row, so this is also the push cap. Four or more stamps
-- collapse; three or fewer keep their individual rows.
...
BEGIN
  ...
  IF coalesce(array_length(v_new_ids, 1), 0) > 3 THEN
    ...
    SELECT
      v_actor,
      NULL,
      concat('Your Passport caught up: ', count(*), ' stamps · +',
        sum(d.points), ' pts'),
      2,
      'admin_message',
      jsonb_build_object(
