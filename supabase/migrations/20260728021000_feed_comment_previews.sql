-- Include the two most recent comments with each feed row.
--
-- The footer shows up to two comment previews. Fetching those per rendered
-- item is exactly the N+1 the feed function exists to remove, so return them
-- inline instead.

DROP FUNCTION IF EXISTS "public"."feed_reviews"("uuid", integer, integer, "uuid", bigint, boolean);
