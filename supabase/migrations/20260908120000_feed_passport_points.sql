...
-- Review and comment avatars rank from permanent Passport points. Keep the
-- active review count for copy, while exposing the live rank source in every
-- embedded profile returned by the cursor feed.

CREATE OR REPLACE FUNCTION public.get_feed_page_v1(
  p_viewer uuid,
  ...
  feed_rows AS (
    SELECT
      ...
      jsonb_build_object(
        ...
        'passport_points', p.passport_points
      ) AS profile,
    ...
    LEFT JOIN LATERAL (
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'profile', jsonb_build_object(
            ...
            'passport_points', preview.profile_passport_points
          ),
      ...
      FROM (
        SELECT
          ...
          cp.passport_points AS profile_passport_points,
          (
            SELECT count(*)
