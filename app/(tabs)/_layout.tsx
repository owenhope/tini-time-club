...
const LayoutContent = () => {
  ...
  const navigateAfterSwipe = useCallback(
    (translationX: number) => {
      const destination = getSwipeTabDestination(pathname, translationX);
  ...
  const swipeGesture = Gesture.Pan()
    // Navigation is a JS-side effect; keeping this gesture on JS also avoids
    // pulling Reanimated into the tab shell just for a route change.
    .runOnJS(true)
    .activeOffsetX([-24, 24])
    .failOffsetY([-30, 30])
    .onEnd(({ translationX }) => {
      if (Math.abs(translationX) < 64) return;
      navigateAfterSwipe(translationX);
    });
