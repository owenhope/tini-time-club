...
Opens on http://localhost:3000. Requires `admin/.env.local`:
...
httpOnly cookie; `proxy.ts` gates every route except `/login`. Single-operator
tool for now — swap for Supabase Auth with an email allowlist when a second
admin exists. The service-role key never leaves the server (`server-only`
imports make client-side use a build error).
